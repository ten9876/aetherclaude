...
const tools = {
    ...
    add_labels:{description:'Add labels to an issue (does not remove existing labels). Pass an array of label names.',inputSchema:{type:'object',properties:{issue_number:{type:'number'},labels:{type:'array',items:{type:'string'}}},required:['issue_number','labels']}},
    remove_label:{description:'Remove a single label from an issue',inputSchema:{type:'object',properties:{issue_number:{type:'number'},label:{type:'string'}},required:['issue_number','label']}},
    close_issue:{description:'Close an issue. state_reason is "completed" (default) or "not_planned".',inputSchema:{type:'object',properties:{issue_number:{type:'number'},state_reason:{type:'string'}},required:['issue_number']}}
};
