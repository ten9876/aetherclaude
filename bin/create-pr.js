...
async function main() {
    ...
    // Attach the public cost-band footer and log a precise audit row.
    // Best-effort — never block PR creation on cost-tracking failure.
    try {
        prBody = execFileSync('/Users/aetherclaude/bin/bot-cost.py', [
            'wrap',
            '--kind', 'pr_description',
            '--target', `pr:issue-${issueNumber}`,
            '--body', '-',
        ], { input: prBody, encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] });
    } catch(e) { /* leave prBody bare */ }
